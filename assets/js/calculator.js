...
// ==============================================
// ENHANCED PROFESSIONAL PDF DOWNLOAD WITH PAGINATION
// ==============================================
function downloadPDF() {
    ...
    try {
        ...
        const drawTable = (data, x, y, width) => {
            const rowHeight = 7; // Reduced from 8 to save space
            let currentY = y;
            ...
            data.forEach((row, index) => {
                ...
                doc.text(row[1], x + width - 5, currentY + 5, { align: 'right' });
        ...
        taxData.forEach(item => {
            ...
            if (item.amount !== undefined) {
                ...
                doc.text(amountStr, pageWidth - 30, y, { align: 'right' });
            }
        ...
        doc.setFontSize(12);
        doc.setTextColor(255, 255, 255);
        doc.text('TOTAL TAX DUE', pageWidth/2 - 35, y + 7);
        doc.text(formatCurrencyPDF(result.totalTax), pageWidth/2 + 35, y + 7, { align: 'right' });
        ...
        doc.setFontSize(9);
        doc.setTextColor(0, 0, 0);
        doc.text(`Effective Rate: ${result.effectiveRate.toFixed(2)}%`, 30, y + 6);
        doc.text(`Net Income: ${formatCurrencyPDF(result.netIncome)}`, pageWidth/2 + 20, y + 6);
        ...
        doc.rect(25, y - 28, pageWidth - 50, 7, 'F');
        doc.setFont("helvetica", "bold");
        doc.setTextColor(30, 130, 76);
        doc.setFontSize(10);
        doc.text('MONTHLY TAKE HOME', 30, y - 22);
        doc.text(formatCurrencyPDF(result.monthlyTakeHome), pageWidth - 30, y - 22, { align: 'right' });
        ...
        doc.text(`${new Date().getFullYear() + 1}`, pageWidth/2 + 30, y);
